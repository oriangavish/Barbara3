...
def calculateDistance(my_iceberg, attack_iceberg):
    my_iceberg.get_turns_till_arrival(attack_iceberg)

# We might want to make there need to be a certain amount of penguins sent during the attack in case they defend their iceberg
...
# If there are multiple icebergs in the same distances, we can send penguins from both at the same time if it's not possible to send penguins from only one and still be able to attack the enemy iceberg
