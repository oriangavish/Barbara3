...
# Then, we'll iterate over the icebergs that are returned,
# and see if we already have a penguin group traveling to that iceberg.
# If not, we choose which iceberg should attack using who_should_attack and
# then use the attack function to send penguin groups from where we specified.
# Function will return True if penguins are successfully sent and False if no attacks occur.
